...
static int jadard_enable(struct drm_panel *panel)
{
	...
	backlight_enable(jadard->backlight);
...
static int jadard_disable(struct drm_panel *panel)
{
	...
	backlight_disable(jadard->backlight);
...
static int jadard_dsi_probe(struct mipi_dsi_device *dsi)
{
	...
	drm_panel_init(&jadard->panel);
	jadard->panel.dev = &dsi->dev;
	jadard->panel.funcs = &jadard_funcs;

	backlight = of_parse_phandle(dev->of_node, "backlight", 0);
	if (backlight) {
		jadard->backlight = of_find_backlight_by_node(backlight);
...
static int jadard_dsi_remove(struct mipi_dsi_device *dsi)
{
	...
	mipi_dsi_detach(dsi);
	drm_panel_remove(&jadard->panel);
	put_device(&jadard->backlight->dev);
